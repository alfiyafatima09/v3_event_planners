...
export const createStarAnimation = (canvas: HTMLCanvasElement) => {
  ...
  const drawStarShape = (x: number, y: number, size: number, opacity: number, sparkleIntensity: number) => {
    ...
    // Reusing a static gradient to avoid recalculating it every frame
    const gradient = ctx.createRadialGradient(x, y, 0, x, y, outerRadius * 1.5);
    gradient.addColorStop(0, `rgba(255, 230, 130, ${opacity * sparkleIntensity})`);
  ...
  const initStars = () => {
    ...
    for (let i = 0; i < numStars; i++) {
      ...
      stars.push({
        ...
        originalX: x,
        originalY: y,
        size: Math.random() * 3 + 1, // Star size between 1 and 4
        brightness: Math.random() * 0.8 + 0.4, // Brightness range from 0.4 to 1.2
  ...
  const updateStar = (star: Star) => {
    ...
    const opacity = star.brightness * (0.6 + twinkle * 0.4); // Brightened opacity
  ...
  const animate = () => {
    ctx.fillStyle = 'rgba(0, 0, 0, 0.05)'; // Light trail effect
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    time += 0.016; // Fixed frame time for smoother animations
